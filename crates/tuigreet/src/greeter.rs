...
impl Greeter {
  pub async fn new(events: Sender<Event>) -> Self {
    ...
    #[cfg(not(test))]
    {
      ...
      if greeter.general.list_outputs {
        tuigreet_system::list_outputs();
      }

      // Handle --dump-config: print effective config as TOML and exit
      if greeter.general.dump_config {
        let toml_str = toml::to_string_pretty(&greeter.config)
...
#[cfg(test)]
mod test {
  use tuigreet_config::config::Config;
  ...
  fn applying_config_replaces_session_state() {
    ...
    let mut configured = Config::defaults();
    ...
    let mut replacement = Config::defaults();
  ...
  fn applying_config_updates_log_file() {
    ...
    assert_eq!(greeter.general.log_file, "/tmp/configured-tuigreet.log");
  }
  ...
  Some(env)) if cmd == "uname" && env.len() == 2)
          ...
          assert_eq!(greeter.layout.prompt_padding, 0);
          assert_eq!(greeter.layout.window_padding, 1);
          assert_eq!(greeter.layout.container_padding, 10);
          assert!(greeter.user_menu.enabled);
          assert_eq!(greeter.session.xsession_wrapper, "startx");
        }),
      ),
      (
        ...
        true,
        Some(|greeter| {
          assert_eq!(greeter.session.xsession_wrapper, "mywrapper.sh");
        }),
      ),
      (
        &["--xsession-wrapper", ""],
        true,
        Some(|greeter| {
          assert_eq!(greeter.session.xsession_wrapper, "");
        }),
      ),
      // Invalid combinations
      (
        &["--secret-mode", "characters", "--secret-characters", ""],
        false,
        None,
      ),
      (
    ...
    for (opts, valid, check) in table {
      // Convert opts to Vec<String> for load_config_from
      let mut args = vec![std::ffi::OsString::from("tuigreet")];
      for opt in opts.iter() {
        args.push(opt.into()); // Then append the flags
      }

      match valid {
        true => {
          // Parse CLI args using load_config_from
          match tuigreet_config::loader::load_config_from(args.clone()) {
            Ok((config, warnings)) => {
              ...
              // Run check closure if present
              if let Some(check_fn) = check {
                check_fn(&greeter);
              }
            },
            Err(err) => {
              // Unexpected success case returned an error
              let (_, errors, _) = *err;
              for e in &errors {
                tracing::error!("{}", e);
              }
              panic!(
                "Unexpected config load error for {:?}: {:?}",
                opts, errors
              );
            },
          }
        ...
        false => {
          // Expect error for invalid combinations
          match tuigreet_config::loader::load_config_from(args) {
            Ok(_) => {
              panic!("Expected config load error for {:?} but got Ok", opts)
            },
            Err(err) => {
              // Expected failure - optionally log for debugging
              let (_, errors, _) = *err;
              for e in &errors {
  ...
  async fn test_session_paths_are_deduplicated_after_config_overlay() {
    // Build args with program name at index 0 (required by load_config_from)
    let args: Vec<std::ffi::OsString> = std::iter::once("tuigreet")
    .chain(["--sessions=/usr/share/wayland-sessions"])
    .map(Into::into)  // ← Convert all &str to OsString
    .collect();

    // Parse CLI args using load_config_from
    let (config, _) = tuigreet_config::loader::load_config_from(args)
      .expect("Expected successful config load");
  ...
  async fn test_config_session_environments_survive_cli_overlay() {
    ...
    // Parse CLI args using load_config_from
    tuigreet_config::loader::load_config_from(args)
      .expect("Expected successful config load");
  ...
  async fn test_config_power_options_are_applied() {
    ...
    assert!(!greeter.power.use_setsid);
